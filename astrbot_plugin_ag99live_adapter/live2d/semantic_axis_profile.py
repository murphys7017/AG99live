...
class SemanticAxisRelationGraph(TypedDict):
    schema_version: str
...
_AXIS_DEFAULTS: dict[str, dict[str, Any]] = {
    "head_yaw": {
        ...
        "soft_range": [34.0, 66.0],
        "strong_range": [14.0, 86.0],
    ...
    "body_yaw": {
        ...
        "semantic_group": "body",
        "control_role": "primary",
        "positive_semantics": ["body turns right", "right-side emphasis", "strong right follow-through"],
    ...
    "body_roll": {
        ...
        "semantic_group": "body",
        "control_role": "primary",
        "positive_semantics": ["body tilts right", "right sway", "playful right lean"],
    ...
    "body_pitch": {
        "label": "Body Pitch",
        "description": "Controls forward/back or vertical-feeling body posture.",
        "semantic_group": "body",
        "control_role": "primary",
        "positive_semantics": ["lift posture", "lean back", "brighten upward"],
        ...
        "output_range": [-10.0, 10.0],
        "soft_range": [36.0, 64.0],
        "strong_range": [20.0, 80.0],
        "extreme_range": [4.0, 96.0],
    },
    "gaze_x": {
        "label": "Gaze X",
    ...
    "eye_smile_left": {
        ...
        "description": "Controls smile/squint shape of the left eye.",
        "semantic_group": "eye",
        "control_role": "primary",
        "positive_semantics": ["left smiling eye", "left squint smile", "playful left eye"],
        "negative_semantics": ["relax left eye smile", "reduce left squint", "neutral left eye"],
    ...
    "eye_smile_right": {
        ...
        "description": "Controls smile/squint shape of the right eye.",
        "semantic_group": "eye",
        "control_role": "primary",
        "positive_semantics": ["right smiling eye", "right squint smile", "playful right eye"],
        "negative_semantics": ["relax right eye smile", "reduce right squint", "neutral right eye"],
    ...
    "mouth_x": {
        "label": "Mouth X",
        "description": "Controls left-right mouth offset for asymmetric expressions.",
        "semantic_group": "mouth",
        "control_role": "hint",
        "positive_semantics": ["mouth shifts right", "right smirk", "asymmetric grin right"],
    ...
    "brow_left_detail": {
        ...
        "description": "Controls left brow detail for asymmetric expression.",
        "semantic_group": "brow",
        "control_role": "hint",
        "positive_semantics": ["raise or relax left brow detail", "left brow accent up"],
    ...
    "brow_right_detail": {
        ...
        "description": "Controls right brow detail for asymmetric expression.",
        "semantic_group": "brow",
        "control_role": "hint",
        "positive_semantics": ["raise or relax right brow detail", "right brow accent up"],
    ...
    "breath": {
        "label": "Breath",
        "description": "Controls breathing or continuous body rhythm.",
        "semantic_group": "breath",
        "control_role": "ambient",
        "positive_semantics": ["stronger breath", "brighter breathing motion"],
        "negative_semantics": ["weaker breath", "calmer breathing motion"],
        "usage_notes": "Owned by ambient/runtime motion; not exposed to LLM prompt by default.",
        "output_range": [0.0, 1.0],
...
def _normalize_relation_graph(
    raw_graph: Any,
    *,
    axis_ids: set[str],
) -> SemanticAxisRelationGraph:
    if not isinstance(raw_graph, Mapping):
        raise SemanticAxisProfileError("SemanticAxisProfile relation_graph must be an object.")
    schema_version = str(raw_graph.get("schema_version") or "").strip()
    if schema_version != SEMANTIC_AXIS_RELATION_GRAPH_SCHEMA_VERSION:
        raise SemanticAxisProfileError(
            "SemanticAxisProfile relation_graph has an unsupported schema_version."
        )
    raw_edges = raw_graph.get("edges")
    if not isinstance(raw_edges, list):
        raise SemanticAxisProfileError("SemanticAxisProfile relation_graph.edges must be an array.")
    ...
    for raw_edge in raw_edges:
        if not isinstance(raw_edge, Mapping):
            raise SemanticAxisProfileError(
                "SemanticAxisProfile relation_graph edge entries must be objects."
            )
        edge_id = _require_non_empty_string(raw_edge.get("id"), field_name="relation_graph.edge.id")
        if edge_id in seen_ids:
            raise SemanticAxisProfileError(
                f"Duplicate semantic axis relation rule id: `{edge_id}`."
            )
        seen_ids.add(edge_id)
        source_axis_id = _require_non_empty_string(
            raw_edge.get("source_axis_id"),
            field_name="relation_graph.edge.source_axis_id",
        )
        target_axis_id = _require_non_empty_string(
            raw_edge.get("target_axis_id"),
            field_name="relation_graph.edge.target_axis_id",
        )
        if source_axis_id == target_axis_id:
            raise SemanticAxisProfileError(
                f"SemanticAxisProfile relation rule `{edge_id}` cannot target its source axis."
            )
        if source_axis_id not in axis_ids or target_axis_id not in axis_ids:
            raise SemanticAxisProfileError(
        ...
        kind = _require_allowed_string(
            ...
            allowed_values=ALLOWED_RELATION_RULE_KINDS,
        )
        ...
        max_delta = _coerce_float(raw_edge.get("max_delta"), field_name="relation_graph.edge.max_delta")
        if min(scale, deadzone, max_delta) < 0.0:
            raise SemanticAxisProfileError(
                f"SemanticAxisProfile relation rule `{edge_id}` contains a negative limit."
            )
        normalized_edge: SemanticAxisRelationRule = {
            "id": edge_id,
    ...
    *,
    ...
    if path.exists():
        ...
        if str(current_profile["source_hash"]).strip() == current_source_hash:
            ...
            if not _profile_requires_regeneration(
...
def _generate_semantic_axis_profile(
    *,
    path: Path,
    model_name: str,
    model_payload: Mapping[str, Any],
...
def _profile_requires_regeneration(
    *,
    current_profile: Mapping[str, Any],
    expected_profile: Mapping[str, Any],
) -> bool:
    if str(current_profile.get("status") or "").strip() == "stale":
        return True
