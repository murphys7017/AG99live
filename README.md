...
**让 AstrBot 拥有一个会回应、会陪伴、会表达的桌面身体。**

Windows 桌面端 · AstrBot · 桌宠式伴侣 · Electron · Live2D Cubism
...
AG99live 是 AstrBot 的桌面身体和 Live2D 表现层：它把 AstrBot 的人格、记忆和对话回应，呈现为一个能长期陪伴用户的桌宠式伴侣。

AG99live 不重新定义 AstrBot 的人格，也不把自己当成独立的虚拟主播。AstrBot 负责理解、决策和回应，AG99live 负责让这些回应拥有可见的身体、声音、动作和状态。
...
AG99live 面向希望让 AstrBot 成为长期桌面陪伴者的开发者和创作者：

- 让 AstrBot 通过桌宠身体与用户持续相处，而不是只能触发固定表情的聊天窗口。
- 让同一套动作语义适配不同 Live2D 模型，并能针对模型能力做细调。
- 观察大模型生成了什么动作、最终为什么变成这个参数，而不是只凭肉眼猜测。
- 把语音、口型、头身随动、物理演算和外部消息入口放进同一条可追踪的交互链路。
- 为后续训练专门的动作生成小模型积累真实的语义动作与播放数据。

它目前首先是一个面向开发和实验的项目，不是一键安装即用的成熟桌宠产品。项目更重视陪伴交互的连续性、身体表达的可解释性、可调试性和持续积累能力。
...
仓库还包含独立的 `vts-data-recorder`：它直接记录本机 VTube Studio 的 tracking input 与 Live2D
参数原始时间序列，并与 Motion Lab 数据库完全隔离。当前只提供发现、采样、录制、查询和删除；
语义标注、审核与训练导出尚未实现，具体边界见
[原始参数录制架构](./docs/05-小模型训练/03-独立VTS原始参数录制架构.md)。

## 技术文档

完整架构和协议从 [文档中心](./docs/README.md) 开始：

- [一期完成边界与二期入口](./docs/01-架构与结构/06-一期完成边界与二期入口.md)
