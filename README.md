...
它和普通聊天软件的区别在于，AG99live 关心的不只是“回复了什么”，还关心角色在回复时应该如何看你、如何转头、如何带动身体、什么时候复用已有动画、什么时候生成新的语义动作。
...
一次回复可以同时包含文本、音频和动作指令。前端播放管线会等待必要的片段到齐，再协调文字显示、音频播放和 Live2D 动作起播，避免角色表现和声音脱节。
...
项目会扫描 Live2D 模型自带的 motion 资源，并整理成可供 AI 选择的动作候选。适合复用已有动画时，系统可以选择模型原本制作好的动作；不合适时，再生成参数化动作。
...
这条链路是项目当前最核心的部分。很多功能，包括动作实验室、语义轴、motion 复用、说话随动，都是围绕这条链路继续打磨角色表现。
