...
目标不是修改引擎，而是先把“哪些参数适合成为动作主轴，哪些适合作为面部辅轴”整理成证据表，后续再据此调整 `SemanticAxisProfile` 默认生成策略、prompt 和调参工具展示。
...
| 衣物/饰品/肢体细碎参数 | `PhyTie*`、`PhyEarring*`、`ParamHairband*`、`ParamForearm*` | 更适合作为 motion 内部效果或后续高级动作层，不进入第一版语义轴 |

## 4. 第一版轴名单

本节是当前准备进入实现阶段的第一版名单。它比“全部候选”更收敛，目标是先得到稳定、可解释、便于调参的一组轴。

### 4.1 第一版动作主轴

第一版动作主轴共 12 个。它们负责姿态、方向、动作强弱和注意力方向。
