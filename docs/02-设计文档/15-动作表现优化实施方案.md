...
因此 `physicsResponseScale` 只改变末端输出幅度，不改变粒子速度、delay、mobility 或 acceleration。设置为 2 后看到摆幅变大是预期行为，但它不会自动变得更灵动。当前实现还会排除 Physics input 以及 SemanticAxisProfile 已绑定的语义姿态参数，避免倍率误放大头部、身体等主动作轴。
...
- SemanticAxisProfile 已绑定的语义姿态参数也不受 terminal scale 影响。
...
当前 `MotionTuningStore.refresh_reference_examples()` 已按 `compiled_semantic_motion.kind` 投影样本：
...
`turn_coordinator.py` 分别写入 `turn.input_received` 和 `turn.assistant_output`，`motion_lab_raw_events` 有 `turn_id`、`message_id`、`user_text`、`assistant_text` 字段及 turn/segment 索引。`MotionLabRawEventStore.get_turn_context()` 已受限读取该轮最近的输入和目标 segment 的输出；`MotionTuningStore` 保存样本时通过该查询填充上下文。前端只提交 `source_record_id + turn_id + message_id`，不能从 UI 当前显示文本猜测，也不应把完整聊天历史重新塞进保存协议。

### 11.3 用户样本按当前语义选择

当前 `resolve_motion_reference_examples()` 已拆分 Prompt 层：

- capability：保留轴定义和少量固定结构示例。
- context：根据当前用户输入选择 1-3 个相关人工样本。
- previous motion：继续只携带上一动作关键轴和动作族。

`AG99liveMotionPromptContributor.collect()` 将当前 event 的用户文本传给 `_build_motion_runtime_payload()`，再使用轻量、确定性的相关性排序：
...
第一版不需要 embedding 或额外小模型。没有相关样本时只使用固定结构示例；这不是动作 fallback，而是 Prompt 本来就允许没有人工样本。每次实际选中并完成 Prompt 能力投影的 `reference_examples` 都以 `motion.prompt_reference_examples_resolved` 写入 Motion Lab 原始事件库，供后续数据复核，不把它们汇总为全局风格文本。
...
- 已完成：按当前用户输入选择相关人工样本，并记录本次实际进入 Prompt 的投影结果。
