...
第一批已完成：`engine.parameter_plan.v3` 在每个参数的 `dynamics.response` 中显式区分
...
批次 A-C 已保持 PlaybackTimeline 和 WebSDK 时钟边界完成；参数计划现已升级为 `engine.parameter_plan.v3`，显式携带部位响应策略。
