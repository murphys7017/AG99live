...
- 最终把所有主动语义输入编译为统一 parameter tracks，再交给逐帧表现层。
...
- ModelEngine 负责等级采样、强度、coupling、timing、关系图和参数绑定。
...
flowchart LR
    ...
    I --> J[ParameterPresentation]
    J --> K[Cubism Motion / Expression base]
    K --> L[Cubism Physics]
    L --> M[Model update / draw]
```
...
| ParameterPresentation | 帧率无关的主动参数动力学与 base value 混合 |
