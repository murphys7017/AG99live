...
-> E2 gaze/face/hesitation response               planned
-> F Physics diagnostics/profile
-> G Motion Lab sequence editor/diversity
-> 有数据后再评估新 sequence 协议和动作小模型
```

批次 A-C 和 E1 已保持 PlaybackTimeline 与 WebSDK 时钟边界完成；参数计划现已升级为
`engine.parameter_plan.v3`，显式携带头部和身体响应策略。下一阶段先完成 D，再根据轨道图需要推进
E2、F 和 G；不新增第二套轨道播放器。
