...
本文定义一期主链路完成后的二期动作表现实施路线。它只描述质量优化，不重新定义产品一期状态。
顺序是统一参数轨道和剩余主动动力学，再建立 Physics 调校与 Motion Lab 深化能力。

产品阶段边界以 [一期完成边界与二期入口](../01-架构与结构/06-一期完成边界与二期入口.md) 为准。

问题依据见 [二期动作与播放优化清单](./14-当前动作与播放问题清单.md)。
...
不会绕过现有 `engine.parameter_plan.v3` 结构。
...
`parameterTrackGraphCompiler` 解决时间关系：
...
第一版没有建立通用可视化图运行时。编译阶段直接把语义步骤和 SpeechPose source 化简为闭合
parameter tracks，逐帧阶段仍只消费 V3 keyframes、modulation、数组和常量。
...
- sequence 只在第一步生成一次 SpeechPose，避免每个 pose 重复规划同一条说话轨迹。
- `ModelParameterBindingStage` 只生成静态参数绑定，不再附着 modulation。
- `ParameterTrackGraphStage` 统一把 SpeechPose modulation 接到对应参数轨道。
- `parameterTrackGraphCompiler` 统一参数集合、权重时长、关键帧和部位时间策略。
- gaze 比步骤基准少量提前，head 使用基准时间，body 延迟并保留更长 residual。
- sequence 保留已有音频或动作总时长；短窗口压缩本轨道 transition/hold，只允许真实末段尾势延长计划。
- 原有 rest-to-rest transition budget 计算和共享 transition 路径已删除。
...
- 输出为 `engine.parameter_plan.v3`；内部图不跨 WebSocket。

## 8. 批次 E：深化主动参数动力学

ParameterPresentation 只处理主动参数，不处理 Physics 粒子。

### 8.1 已完成：头部与身体 V3 响应策略

`engine.parameter_plan.v3` 已在每个参数的 `dynamics.response` 中显式区分
`bounded` 与 `spring`。ModelEngine 根据 semantic group 编译策略，ParameterPresentation 只执行：

- head：`spring`，`2.4Hz`，阻尼比 `0.78`；较快起势并保留轻微受控过冲。
- body/torso/shoulder：`spring`，`1.0Hz`，阻尼比 `0.85`；更慢启动、反转和收势。
- 其它参数：`bounded`，继续使用原有最大速度/最大加速度精确制动。

spring 响应仍受参数自己的 `max_velocity` 与 `max_acceleration` 限制；Parameter Mixer 继续只做
跨来源融合与最终裁剪，Cubism Physics 输出不经过该响应层。Timeline 增量按不大于 `1/120s`
的确定性子步积分，窗口卡顿或音频时钟较大步进时不丢弃时间，也不会用一个大步长制造异常过冲。
...
头部和身体的基础 spring 响应已经落地。后续根据参数时间轨道图和实际动作数据，再补充：
...
-> D parameter track graph                       complete
-> E1 head/body V3 response dynamics             complete
-> E2 gaze/face/hesitation response               next
-> F Physics diagnostics/profile
-> G Motion Lab sequence editor/diversity
-> 有数据后再评估新 sequence 协议和动作小模型
```

批次 A-D 和 E1 已保持 PlaybackTimeline 与 WebSDK 时钟边界完成；参数计划继续使用
`engine.parameter_plan.v3`。下一阶段根据真实轨道数据推进 E2，再进入 F 和 G；不新增第二套轨道播放器。
