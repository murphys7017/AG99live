...
- `PlaybackTimelineRuntime.startSegmentJob()` 已成为段级 start 入口；text/audio/motion/session segment sinks 先注册到 runtime，缺失 sinks 会作为 wiring 错误直接抛出；`useTurnPlaybackOrchestrator` 不再直接调用 executor，也不再携带具体 sink 实现。
...
- `audioPlaybackStateBridge` 和 `MotionSink` 的 timeline wiring 也已经改为必需依赖；音频 started / duration / terminal、motion timeline lookup / prepare / motion-only prepare 不能再因为缺少回调而静默绕过统一 timeline。
