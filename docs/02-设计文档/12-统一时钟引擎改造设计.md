...
- `turnPlaybackOrchestratorCore` 已把分散的 text/audio/motion release 回调收敛成带 `text/audio/motion` 槽位的 `releaseSegment(job)`；`segmentReleaseExecutor` 当前负责把 job 映射到现有文本、音频、动作端口，并在 motion 缺少合法 `receivedAtMs` 时显式报错；下一步可以把这个 executor 替换为 timeline runtime 的 segment job 执行入口。
