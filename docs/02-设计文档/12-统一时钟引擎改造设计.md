...
> 文档状态：演进中。Phase 1 纯逻辑 timeline 已实现；Phase 2 已把真实音频播放接入 timeline；Phase 3 已把 motion release context、audio-start event、motion-only synthetic timeline 和 motion terminal 回写接入统一 timeline；AudioSink / MotionSink / LipSyncSink 适配层已落地，motion 作为 required sink、lip sync 作为 optional sink 进入 PlaybackTimelineEngine 的 terminal 集合；下一步是把剩余 release / event 分发继续收进 timeline。
>
...
- `lip_sync` 已作为 optional sink 注册进音频 segment 的 timeline。AudioSink 仍负责 attach/resume/stop LipSyncSink，但 started / failed / completed 会回写到 `PlaybackTimelineEngine` 的 sink terminal 集合。
...
- 保留 `notifyAudioPlaybackStarted` 作为 ModelEngine 内部调度入口，但触发源改为 audio timeline started handler，而不是 CompletionCoordinator 的 session watch。
...
- `turnPlaybackOrchestratorCore` 已把分散的 text/audio/motion release 回调收敛成带 `text/audio/motion` 槽位的 `releaseSegment(job)`；`segmentPlaybackRelease` 当前负责把 job 映射到现有文本、音频、动作端口，并在 motion 缺少合法 `receivedAtMs` 时显式报错；下一步可以把这个 job 直接交给 timeline runtime。
...
- `MotionTimelineSink` 会只在 `turnId/messageId` 严格匹配时把 active audio timeline snapshot 附加到 motion context，不匹配时显式传 `null`。
...
- `AdapterAudioRuntime` 在真实 audio timeline 进入 `playing` 后发布 audio-start snapshot；`usePetDesktopRuntime` 保留 90ms 动作延迟策略，并把该 snapshot 传给 ModelEngine。
...
- 当 segment 明确无音频但有 motion payload 时，`MotionTimelineSink` 会请求创建 motion-only synthetic timeline，并把该 snapshot 传给 `ModelEngine`；motion started 会启动 synthetic clock，motion terminal 会收口 timeline。
...
- `notifyAudioPlaybackStarted` 方法仍保留，用于迁移期唤醒 pending motion 和触发 speech-only motion；但它现在由 audio timeline started handler 调用，CompletionCoordinator 不再负责 audio-start 通知。

验收：

- 动作不再依赖内部“等音频起播”的猜测作为主路径。
- late motion 能读取 timeline currentTime 和 remaining duration。
- speech-only motion 在迁移期由 audio timeline started event 触发，必须使用匹配的 timeline snapshot，而不是重新从 SessionStore 猜音频时长。
...
- `playback-timeline/audioSink.ts` 在音频元素创建时 attach LipSyncSink，在音频起播时 resume，在 audio dispose / stop 时清理嘴型，并把 lip sync started / failed / completed 作为显式回调上报。
...
- LipSyncSink 当前仍由 AudioSink 组合执行，还不是由 `PlaybackTimelineEngine` 直接调用 `prepare/start` 的独立执行器；但它的生命周期结果已经进入统一 terminal 收集。
