...
1. 继续收敛 segment-sink wiring：audio / motion start 已由 `PlaybackTimelineEngine.startSink()` 触发；app-facing port 已不再暴露 raw queued-audio consumer 或 sink wiring。下一步让 audio / motion terminal 与 SessionStore 回写更靠近 sink lifecycle，并让 `PlaybackTimelineRuntime.startSegmentJob()` 只保留 segment job 入口和 SessionStore 回写协调。
