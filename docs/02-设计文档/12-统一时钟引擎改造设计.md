...
- adapter-facing playback port 不再暴露 `prepareSegmentJob()` / `prepareMotionOnlyTimeline()` / `prepareMotionTimelineSink()`；外部不能绕过 `startSegmentJob()` 直接准备 motion timeline。`prepareAudioTimeline()` 暂时保留给 audio segment lifecycle runner 使用，因为真实 audio element 事件仍需要先建立 audio/lip_sync timeline。
...
- LipSyncSink 当前仍随 audio segment playback sink 的音频生命周期被触发，还不是由 `PlaybackTimelineEngine` 直接调用 `prepare/start` 的独立执行器；但 AudioSink 本体和 audio playback action 都不再持有嘴型状态机、音频元素事件排序或 Live2D 参数写入细节，LipSync sink 的生命周期结果已经进入统一 terminal 收集。
- 下一步是把 lip_sync 也提升成更独立的 timeline sink lifecycle：timeline 负责 start/terminal 调度，Live2D 边界继续负责采样和参数写入。
...
1. 继续收敛 segment-sink wiring：audio / motion start 已由 `PlaybackTimelineEngine.startSink()` 触发；下一步让 audio / motion terminal 与 SessionStore 回写更靠近 sink lifecycle，并让 `PlaybackTimelineRuntime.startSegmentJob()` 只保留 segment job 入口和 SessionStore 回写协调。
2. 独立 lip sync：低层 lip sync 采样和 Live2D 参数写入已迁到 Live2D 边界；下一步让 lip_sync 的 start/terminal 更像独立 optional sink，而不是继续挂在 audio segment sink 的事件顺序里。
