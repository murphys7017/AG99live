...
1. 继续收敛 segment-sink wiring：audio / motion start 已由 `PlaybackTimelineEngine.startSink()` 触发，audio `duration/started/terminal` 的 active-timeline SessionStore 写入已进入 `PlaybackTimelineRuntime`；下一步让 motion terminal 与剩余 SessionStore 回写更靠近 sink lifecycle，并让 `PlaybackTimelineRuntime.startSegmentJob()` 只保留 segment job 入口和必要协调。
