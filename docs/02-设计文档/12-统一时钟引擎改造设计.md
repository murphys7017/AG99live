...
`output.segment.v2` 会一次性提交音频和动作材料。Timeline 不等待材料补写，
但允许动作 sink 在音频真正进入 `playing` 后按同一时钟延后启动。

```text
timeline prepares AudioSink + MotionSink
audio enters playing at t = 0ms
MotionSink starts at t = 90ms
ModelEngine compiles with:
  currentTimeMs = audio clock currentTime
  remainingDurationMs = durationMs - currentTimeMs
```
