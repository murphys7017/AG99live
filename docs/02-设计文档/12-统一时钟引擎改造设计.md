...
- 音频 `duration` / `started` 的 SessionStore 写入已移到 `PlaybackTimelineRuntime` 的 audio lifecycle session port；`audioPlaybackStateBridge` 只更新 adapter UI 播放状态，不再直接写这两个会话事实。音频 terminal 仍暂时留在 adapter settlement/terminal 处理里，下一步需要单独收敛以避免 stop/interrupt 双写。
...
1. 继续收敛 segment-sink wiring：audio / motion start 已由 `PlaybackTimelineEngine.startSink()` 触发，audio `duration/started` 的 SessionStore 写入已进入 `PlaybackTimelineRuntime`；下一步让 audio terminal、motion terminal 与剩余 SessionStore 回写更靠近 sink lifecycle，并让 `PlaybackTimelineRuntime.startSegmentJob()` 只保留 segment job 入口和必要协调。
