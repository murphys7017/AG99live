...
当前确认的目标方向是让 `PlaybackTimelineEngine` 直接驱动 sink 的 `prepare/start/terminal` 语义，而不是长期保留迁移期的 segment ports。当前 audio/motion 的 start 已经由 engine 的 sink callback 触发；`PlaybackTimelineRuntime.startSegmentJob()` 可以继续作为过渡入口，但最终应变成“把 segment job 转交给 engine 执行，并协调 SessionStore 回写”的薄层。
...
1. 继续收敛 segment ports：audio / motion start 已由 `PlaybackTimelineEngine.startSink()` 触发；下一步让 prepare/terminal 更靠近 sink lifecycle，并让 `PlaybackTimelineRuntime.startSegmentJob()` 只保留 segment job 入口和 SessionStore 回写协调。
