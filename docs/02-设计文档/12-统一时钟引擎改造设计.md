...
- `live2d/lipSyncTimelineSink.ts` 已接管 WAV lip sync 预加载、live WebAudio RMS 分析、`setExternalLipSyncValue` 写入和 `clearExternalLipSyncValue` 清理，把低层嘴型采样与 Live2D 参数写入移到 Live2D 边界。
...
- 后续如果继续深化，只应把 `lip_sync` 的 start callback 注册为更独立的 optional sink lifecycle；低层 WAV/live RMS 采样、`setExternalLipSyncValue` 写入和清理仍留在 Live2D 边界，不回流到 adapter 或 ModelEngine。
