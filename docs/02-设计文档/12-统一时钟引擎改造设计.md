...
`AudioSink` 已从 adapter runtime 中抽离，当前由 `playback-timeline/audioSink.ts` 持有浏览器 `HTMLAudioElement` 播放实现；`playback-timeline/audioSegmentTimelineRunner.ts` 负责把音频 segment 的 started / duration / terminal 事件回写到 `PlaybackTimelineRuntime`，`audioPlaybackActions.ts` 只保留 adapter/session 状态回写。
