...
- `turnPlaybackOrchestratorCore` 已把分散的 text/audio/motion release 回调收敛成带 `text/audio/motion` 槽位的 `releaseSegment(job)`；segment job 执行语义已移动到 `playback-timeline/segmentJobExecutor.ts`，`useTurnPlaybackOrchestrator` 现在直接把 job 转交给 `PlaybackTimelineRuntime.startSegmentJob()`。
- `PlaybackTimelineSegmentExecutor` 现在仍调用现有 text/audio/motion sink 实现，但已经统一决定 motion release context，例如显式 no-audio 时写入 `timelineMode=motion_only`，motion 缺少合法 `receivedAtMs` 时立即失败。
...
- `PlaybackTimelineSegmentExecutor` / `useTurnPlaybackOrchestrator` 的 timeline runtime port 已经是必需依赖；迁移期不再允许缺少统一时钟 runtime 时静默绕过 timeline preparation。
...
- 旧的 `turn-playback/segmentPlaybackRelease.ts` 空转层已移除；core 产出的 segment job 现在直接进入 `PlaybackTimelineRuntime.startSegmentJob()`。下一步可以继续把 audio/text/motion sink 的实际启动实现从现有 adapter facade 收进 timeline runtime 内部 sink。
...
- `playback-timeline/audioSink.ts` 已拆成纯 `createBrowserAudioTimelineSink()` 和 `createAudioTimelineSinkWithLipSync()` 包装器：纯 AudioSink 只负责 HTMLAudioElement 生命周期、事件和 clock，LipSync 包装器负责 attach/resume/terminal/cleanup。
...
- LipSyncSink 当前仍通过 audio sink wrapper 随音频生命周期启动，还不是由 `PlaybackTimelineEngine` 直接调用 `prepare/start` 的独立执行器；但 AudioSink 本体已经不再持有嘴型状态机，它的生命周期结果也已经进入统一 terminal 收集。
