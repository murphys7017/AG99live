...
> 文档状态：统一时钟主链路已完成迁移。正式回复现在通过原子 `output.segment` 进入 SessionStore，Orchestrator 只生成 SegmentJob；AudioSink、MotionSink 与 LipSyncSink 均由构造时固定的 PlaybackTimelineRuntime 管理。本文中的 late-motion、分片等待和渐进迁移描述仅保留为历史背景，不再代表当前协议。
>
...
当前确认的目标方向是让 `PlaybackTimelineEngine` 直接驱动 sink 的 `prepare/start/terminal` 语义，而不是长期保留迁移期的 segment-sink wiring。当前 audio/motion 的 start 已经由 engine 的 sink callback 触发；`PlaybackTimelineRuntime.startSegmentJob()` 可以继续作为过渡入口，但最终应变成“把 segment job 转交给 engine 执行，并协调 SessionStore 回写”的薄层。
...
LipSyncSink
  -> 使用同一个 timeline 对齐嘴型
  -> 从当前 HTMLAudioElement 的 Web Audio 图读取实时音频能量
...
- 归一化 motion payload
- 编译 `engine.motion_intent.v4`（主链路）或 `engine.motion_intent.v3`（官方兼容/手动预览）
...
frontend/src/playback-integrations/
  modelEngineMotionSink.ts
```
...
`AudioSink` 已从 adapter runtime 中抽离，当前由 `playback-timeline/audioSink.ts` 持有浏览器 `HTMLAudioElement` 播放实现；`playback-timeline/audioSegmentTimelineRunner.ts` 负责把音频 segment 的 started / duration / terminal 事件回写到 `PlaybackTimelineRuntime`，`audioPlaybackStateBridge.ts` 只保留 adapter/session 状态回写。
...
- 触发 canonical subtitle / terminal 状态回写
...
旧：
  releaseAudioForPlayback()
    -> adapter-owned audio playback action
    -> markAudioStarted()
    -> ModelEngine audio-start guess

新：
  PlaybackTimelineRuntime.startSegmentJob(job)
...
`LipSyncSink` 应从音频播放函数附近抽出来，成为 timeline sink。更细的底层实现可以放到 Live2D WebSDK adapter/runtime 边界：timeline 负责调度 lip sync 生命周期，Live2D WebSDK 侧负责 WebAudio graph、mouth value sampling、Live2D external lip sync 参数写入和清理。

职责：

- 使用同一个 audio source / audio clock
- 管理绑定当前 HTMLAudioElement 的 live RMS
...
下一步先做少量预设逐帧曲线，而不是马上让小模型实时生成完整曲线。小模型或 hint 只选择预设，例如：

```text
soft_in_hold_out
quick_react_slow_release
breath_pulse
```
...
`output.segment.v3` 会一次性提交唯一 canonical text、音频和动作材料。Timeline 不等待材料补写。Audio metadata 给出 duration 时，ModelEngine 先完成语义计划编译；Audio 真正进入 `playing` 时，Timeline 从同一个起播事实释放字幕、启动口型并唤醒动作。

```text
timeline prepares AudioSink + MotionSink
audio metadata resolves durationMs
ModelEngine prepares parameter plan with full durationMs
audio enters playing at t = 0ms
subtitle + LipSyncSink + prepared MotionSink start from the same event
performance curve controls the motion's visual entry timing
```

这样动作延迟只属于播放时序，不再表现为独立 motion payload 晚到后补写 Store。
...
调用 PlaybackTimelineRuntime/PlaybackTimelineEngine 启动 segment
```

文本可以保持由 orchestrator 释放，也可以作为 `TextSink` 后续接入 timeline。当前迁移方向是继续把“怎么释放、怎么启动、怎么终止”收进 timeline runtime/engine，让 orchestrator 逐步只负责发现 segment ready。
...
- 抽离旧 adapter-owned audio playback action 的核心播放能力。
- 让 timeline 创建 AudioSink 并接管 audio started / terminal。
- 保持现有 `SessionStore` 回写行为。

当前进展：

- `playback-timeline/audioSink.ts` 已接管浏览器 HTMLAudioElement 播放实现；每次 `playing` 事件携带绑定本次 AudioElement 的 `AudioPlaybackClock`，不存在全局 active audio clock 查询。
- `playback-timeline/audioSink.ts` 已成为真实音频播放适配层；`AdapterAudioRuntime` 不再持有 raw HTMLAudioElement 或 clock 查询，只通过 `PlaybackTimelineAudioSink` 接收分段音频生命周期事件。
- app composition root 直接创建并持有唯一 `PlaybackTimelineRuntime` 与 `BrowserAudioTimelineSink`；adapter 只提供 queued segment、SessionStore projection 等执行 ports，并在 App 显式 attach 后转发音频生命周期。
...
- `PlaybackTimelineRuntime.startAudioTimelineSink()` 已成为 audio sink start 的 runtime 入口；`playback-timeline/audioSegmentTimelineRunner.ts` 通过该入口注册并启动 audio sink，再把真实音频的 duration / started / terminal 事件回写到 runtime。adapter action 不再直接调用 audio timeline duration / started / terminal mark 函数。
- `playback-timeline/audioReleaseController.ts` 已接管音频 pending queue / release 到 segment sink 的通用逻辑；adapter runtime 只在队列接收时回写展示文案，并在 release 后进入 timeline audio runner。
- `loadedmetadata / playing / ended / error / stop` 会同步更新 audio timeline。
- 现阶段 timeline 作为音频执行事实来源，并通过 MotionSink / LipSyncSink 适配层对齐 `ModelEngine` 与 Live2D 嘴型；但尚未替代 `useTurnPlaybackOrchestrator` 的释放决策。audio / motion 的 start 已由 `PlaybackTimelineEngine.startSink()` 触发，lip sync 仍随 audio segment sink 的音频生命周期协调。
- `lip_sync` 已作为 optional sink 注册进音频 segment 的 timeline。`PlaybackTimelineAudioSegmentSink` 现在组合纯 AudioSink 与独立 `PlaybackTimelineAudioLipSyncSink`：AudioSink 暴露音频元素生命周期，LipSync sink 负责 attach/start/terminal/stop，并把 started / failed / completed 回写到 `PlaybackTimelineEngine` 的 sink terminal 集合。
...
- `turnPlaybackOrchestratorCore` 已把分散的 text/audio/motion release 回调收敛成带 `text/audio/motion` 槽位的 `releaseSegment(job)`；segment job 执行语义已收进 `PlaybackTimelineRuntime.startSegmentJob()`，`playback-timeline/segmentJob.ts` 现在只保留 job 和 execution ports 类型定义。
- `PlaybackTimelineRuntime.startSegmentJob()` 现在统一决定 segment release 语义和 motion release context，例如显式 no-audio 时写入 `timelineMode=motion_only`，motion 缺少合法 `receivedAtMs` 时立即失败；audio/motion 的真实 start 由 `PlaybackTimelineEngine.startSink()` 调用已注册的 sink callback。
- `PlaybackTimelineRuntime.startSegmentJob()` 已成为段级 start 入口，并在 runtime 内部完成 motion timeline preparation：audio segment 会复用同一 `turnId/messageId` 的 audio timeline 并注册 required motion sink；显式 no-audio motion segment 会准备 motion-only synthetic timeline。motion-only timeline 准备失败时直接标记 `motion_only_timeline_unavailable`，不继续把 payload 交给 ModelEngine 兜底播放。
- adapter-facing playback port 不再暴露 `prepareSegmentJob()` / `prepareMotionOnlyTimeline()` / `prepareMotionTimelineSink()` / raw queued-audio release；外部不能绕过 `startSegmentJob()` 直接准备 motion timeline 或消费音频队列。`prepareAudioTimeline()` 已退回 `PlaybackTimelineRuntime` 内部私有实现，真实 audio element 启动也必须经 `startAudioTimelineSink()` 进入 `PlaybackTimelineEngine.startSink()`。Timeline 实例由 App 组合根直接创建，Adapter 不再接收 runtime factory 或拥有实例创建权。
- text/audio/motion/session execution ports 先配置到 runtime，缺失 ports 会作为 wiring 错误直接抛出；`useTurnPlaybackOrchestrator` 不再直接调用 executor，也不再携带具体 sink 实现。
- `playback-timeline/segmentReleaseSinks.ts` 已接管 queued text/audio segment 的 pending item 取出、turn/message 校验、队列删除和启动副作用触发；pending playback 队列工具也已迁到 `playback-timeline/playbackReleaseQueue.ts`。adapter 侧仍持有 pending 队列和 UI/history/audio playback 副作用函数，但只向 App 提供一次性构造 Timeline 所需的 immutable execution ports；audio release port 在显式 attach 后调用 `releaseQueuedAudioForTimelinePlayback()`，不存在运行中的 `configureSegmentExecution()` 或第二个 `markAudioReleased` 写入者。
- `useTurnPlaybackOrchestrator` 的 timeline runtime port 已经是必需依赖；迁移期不再允许缺少统一时钟 runtime 时静默绕过 timeline preparation。
- `audioPlaybackStateBridge` 和 `MotionSink` 的 timeline wiring 也已经改为必需依赖；音频 started / duration / terminal 与 motion timeline lookup / terminal 不能再因为缺少回调而静默绕过统一 timeline，motion preparation 只允许由 runtime 的 segment job 执行路径触发。
- 音频 `duration` / `started` / `terminal` 的 SessionStore 写入只经过 `PlaybackTimelineRuntime` 内部 `sessionProjection`；`audioPlaybackStateBridge` 只更新 adapter UI 播放状态。尚未进入 sink 的 pending audio 由 adapter settlement 通知 `rejectAudioBeforeStart()`，仍由 Timeline 解释并写入唯一失败终态。
- motion `started` / `terminal` 同样只经过 Timeline 的 `sessionProjection`。ModelEngine 只报告 pre-start rejection 或播放器 lifecycle，不能读取或直接写 SessionStore；`PlaybackCompletionCoordinator` 只负责 turn-level `playback_finished` ACK 和 completed/failed phase。
- audio / lip_sync / motion 的 duration / started / terminal 事件不再隐式补注册缺失 sink，也不会写入不匹配的 timeline；required lifecycle 事件遇到未关闭 segment 的 timeline/sink 缺失时会记录 wiring error，并把对应 audio/motion slot 显式标为 failed。只有已确认关闭后的迟到事件允许记录警告后丢弃。
- 旧的 `turn-playback/segmentPlaybackRelease.ts` 空转层已移除；core 产出的 segment job 现在直接进入 `PlaybackTimelineRuntime.startSegmentJob()`，并由 runtime 内部完成 timeline preparation、session 标记和 engine-driven sink start。下一步继续把 prepare/terminal 和 remaining segment port 职责收敛成更完整的 sink lifecycle。
- `playback-timeline` 不再直接依赖 `model-engine`；timeline core 只保留 `motionTypes.ts` 这类纯协议类型，`ModelEngine` 的 motion sink / run tracker 适配层已移到 `playback-integrations/modelEngineMotionSink.ts`。`useTurnPlaybackOrchestrator` 不再读取 active audio timeline，只负责 release motion。
...
- 固定 audio-start 延迟桥已删除。`playback-timeline/audioMotionStartBridge.ts` 在 `playing` 事件中重新读取 segment-scoped timeline snapshot，并立即启动已经准备好的计划。
- `adapter-connection/runtime/audioSettlementController.ts` 已接管 pending audio 查询、缺失音频标 absent、按 turn/all 结算 pending/open audio 的逻辑；`AdapterAudioRuntime` 继续收缩为 playback release、timeline、settlement controller 的装配 facade。
- 音频 `playing` 回调必须携带该 segment 的 AudioElement clock；缺失 clock 属于类型和 wiring 错误，不能查询全局 active audio 或伪装成 synthetic。
...
- `AdapterAudioRuntime` 在 audio duration ready 时发布准备 snapshot，在真实 audio timeline 进入 `playing` 后发布启动 snapshot；ModelEngine 前者编译、后者播放，不再持有固定毫秒延迟策略。
...
- motion interrupt 必须携带 `turn_id + message_id`。ModelEngine 只允许中断匹配该段的 active run，并同步删除该段 pending payload；禁止旧 Timeline 通过全局 `stop()` 误停新段动作。
...
- 当没有 queued motion 但需要 speech-only motion 时，`ModelEngine` 接收内部 `speech_only` request，只在 `turnId/messageId` 严格匹配时使用 audio-start timeline snapshot 求剩余时长和 `speechActive`；该请求不会伪造生产 `engine.motion_intent.v3/v4` payload。
...
- 动作不再依赖内部“等音频起播”的猜测。
- 延后启动的 motion 能读取 timeline currentTime 和 remaining duration。
...
- `playback-timeline/lipSyncSink.ts` 只保留 `PlaybackTimelineLipSyncRuntime` 和 sink contract，负责 lip sync started / unavailable / completed / failed 的终态规则，避免音频播放适配层继续持有嘴型状态机。
- `live2d/lipSyncTimelineSink.ts` 已接管 live WebAudio RMS 分析、`setExternalLipSyncValue` 写入和 `clearExternalLipSyncValue` 清理，把低层嘴型采样与 Live2D 参数写入移到 Live2D 边界。生产播放不再并行启动独立推进的 WAV handler。
- `adapter-connection/runtime/audioPlayback.ts` 已删除；HTMLAudioElement 生命周期和 clock 暴露现在归属 `playback-timeline/audioSink.ts`。
- `playback-timeline/audioSink.ts` 已拆出纯 `createBrowserAudioTimelineSink()`：纯 AudioSink 只负责 HTMLAudioElement 生命周期、事件和 clock；LipSync runtime 由 `adapter-connection/runtime/audioTimelineController.ts` 装配进独立 `PlaybackTimelineAudioLipSyncSink`，再由 `PlaybackTimelineAudioSegmentSink` 统一组合 audio element 事件和 lip-sync lifecycle。
...
- LipSyncSink 当前仍随 audio segment playback sink 的音频元素事件被 attach/start/terminal/stop；这是为了保证嘴型使用同一个 HTMLAudioElement 和同一条音频 clock。AudioSink 本体和 audio playback action 不再持有嘴型状态机、音频元素事件排序或 Live2D 参数写入细节，LipSync sink 的生命周期结果已经进入统一 terminal 收集。
- 后续如果继续深化，只应把 `lip_sync` 的 start callback 注册为更独立的 optional sink lifecycle；低层 live RMS 采样、`setExternalLipSyncValue` 写入和清理仍留在 Live2D 边界，不回流到 adapter 或 ModelEngine。

验收：

- 音频开始后嘴型立即对齐声音。
- interrupt 后嘴型不会残留。
- live RMS 必须绑定 Timeline 当前持有的 HTMLAudioElement；能力不可用时必须上报 `lip_sync` failed，不允许切换到独立推进的 WAV 时钟或静默成功。
...
- 后续先支持少量预设逐帧曲线并绑定 timeline tick；小模型/curve hint 只选择预设和少量强度参数。真实实时生成任意曲线放到预设方案验证之后。
...
- 同一原子段内音频先启动、动作延后：motion 使用 timeline currentTime。
- 原子段重复提交：协议边界直接拒绝，不重复播放。
...
- `output.segment` 不允许 late audio 补写；缺失音频必须在原子段中明确声明 `absent` 或 `failed`。
...
当前骨架、AudioSink、MotionSink、LipSyncSink 适配层和 segment job runtime 已落地。下一步按风险从小到大推进：

1. 继续拆分 `PlaybackTimelineRuntime` 内部热点，但不得拆出第二个状态机：当前 `sessionProjection` 已统一 SessionStore 投影，后续只按 registry、sink lifecycle 和 deferred subtitle 等纯职责拆文件。
2. 独立 lip sync：低层 lip sync 采样和 Live2D 参数写入已迁到 Live2D 边界；下一步只评估是否把 lip_sync 的 start/terminal 注册成更独立的 optional sink callback，不能把 WebAudio/Live2D 参数写入重新散回 adapter。
3. 预设逐帧曲线：先实现 3-5 个 preset curve，并让现有 performance curve hint 选择 preset；不要直接做任意实时曲线生成。
