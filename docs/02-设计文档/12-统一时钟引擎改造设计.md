...
> 文档状态：演进中。Phase 1 纯逻辑 timeline 已实现；Phase 2 已把真实音频播放接入 timeline；Phase 3 已开始把 motion release context 接入 audio timeline，但 MotionSink / LipSyncSink 尚未完全迁入。
>
...
- 现阶段 timeline 作为音频执行事实来源，并通过 MotionSink 适配层向 `ModelEngine` 提供 matched snapshot；但尚未替代 `useTurnPlaybackOrchestrator` 的释放决策，也尚未由 `PlaybackTimelineEngine` 直接驱动 MotionSink / LipSyncSink 生命周期。
...
- 暂保留当前 `notifyAudioPlaybackStarted` 迁移路径，直到 timeline 接管同一 segment 的音频起播事件。
...
- 音频起播通知会用同一 `turnId/messageId` 的最新 timeline snapshot 刷新 pending motion，避免 release 时拿到的旧 snapshot 影响动作时长。
- 当没有 queued motion 但需要 speech-only motion 时，`ModelEngine` 也会只在 `turnId/messageId` 严格匹配时使用 audio-start 携带的 timeline snapshot 来求剩余时长和 `speechActive`。
- 旧的 `notifyAudioPlaybackStarted` 仍保留，用于迁移期承载 audio-start timeline snapshot、唤醒 pending motion 和触发 speech-only motion；后续应由 timeline event / MotionSink 替代。

验收：

- 动作不再依赖内部“等音频起播”的猜测作为主路径。
- late motion 能读取 timeline currentTime 和 remaining duration。
- speech-only motion 在迁移期由 audio-start 通知触发，但必须使用匹配的 timeline snapshot，而不是重新从 SessionStore 猜音频时长。
