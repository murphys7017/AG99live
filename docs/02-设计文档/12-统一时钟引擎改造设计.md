...
- `playback-timeline/audioSegmentTimelineRunner.ts` 已接管音频 segment 播放事件到 timeline runtime 的桥接，adapter action 不再直接调用 audio timeline duration / started / terminal mark 函数。
...
- LipSyncSink 当前仍随 audio segment playback sink 的音频生命周期被触发，还不是由 `PlaybackTimelineEngine` 直接调用 `prepare/start` 的独立执行器；但 AudioSink 本体和 audio playback action 都不再持有嘴型状态机、音频元素事件排序或 runtime/callback wiring，LipSync sink 的生命周期结果已经进入统一 terminal 收集。
- 下一步评估把 WebAudio RMS / WAV 分析 / external lip sync 参数写入下沉到 Live2D WebSDK adapter/runtime 中；timeline 侧保留 lip_sync optional sink 的生命周期和 terminal 上报。
...
1. 继续收敛 segment-sink wiring：audio / motion start 已由 `PlaybackTimelineEngine.startSink()` 触发；下一步让 prepare/terminal 更靠近 sink lifecycle，并让 `PlaybackTimelineRuntime.startSegmentJob()` 只保留 segment job 入口和 SessionStore 回写协调。
2. 独立 lip sync：评估将低层 lip sync 采样和 Live2D 参数写入迁到 Live2D WebSDK adapter/runtime，timeline 只调度生命周期和 terminal。
