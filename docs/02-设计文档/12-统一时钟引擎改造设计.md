...
- `turn-playback/segmentPlaybackRelease.ts` 仍是迁移期 compatibility adapter，但已经把 adapter 的 timeline runtime port 传给 `playback-timeline/segmentJobExecutor.ts`。下一步可以继续把 audio/text/motion sink 的实际启动从现有 adapter facade 收进 `PlaybackTimelineRuntime`。
