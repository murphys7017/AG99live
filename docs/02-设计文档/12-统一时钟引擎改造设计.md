...
- 现阶段 timeline 只作为音频执行事实来源，尚未替代 `useTurnPlaybackOrchestrator` 的释放决策，也尚未驱动 MotionSink / LipSyncSink。
...
- `PlaybackReleasePort` 暴露只读的 `getActiveAudioTimelineSnapshot()`，motion release 只在 `turnId/messageId` 匹配时把该快照附加到 `TurnPlaybackReleaseContext`。
...
- 旧的 `notifyAudioPlaybackStarted` 仍保留，用于迁移期唤醒 pending motion 和 speech-only motion；后续应由 timeline event 替代。
