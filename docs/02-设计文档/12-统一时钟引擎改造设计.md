...
> 文档状态：演进中。Phase 1 纯逻辑 timeline 已实现；Phase 2 已把真实音频播放接入 timeline；Phase 3 已把 motion release context、audio-start event、motion-only synthetic timeline、motion terminal 回写和 segment job preparation 接入统一 timeline；AudioSink / MotionSink / LipSyncSink 适配层已落地，motion 作为 required sink、lip sync 作为 optional sink 进入 PlaybackTimelineEngine 的 terminal 集合；下一步是从迁移期 segment ports 继续收敛到由 PlaybackTimelineEngine 直接驱动各 sink 的 prepare/start/terminal。
>
...
当前确认的目标方向是让 `PlaybackTimelineEngine` 直接驱动 sink 的 `prepare/start/terminal` 语义，而不是长期保留迁移期的 segment ports。`PlaybackTimelineRuntime.startSegmentJob()` 可以继续作为过渡入口，但最终应变成“把 segment job 转交给 engine 执行”的薄层。
...
- 旧的 `turn-playback/segmentPlaybackRelease.ts` 空转层已移除；core 产出的 segment job 现在直接进入 `PlaybackTimelineRuntime.startSegmentJob()`，并由 runtime 内部完成 timeline preparation、session 标记和 sink 启动。下一步继续把迁移期 segment ports 收敛成 engine 直接驱动的 sink lifecycle。
...
1. 收敛 segment ports：让 `PlaybackTimelineEngine` 直接驱动 audio / motion sink 的 prepare/start/terminal，`PlaybackTimelineRuntime.startSegmentJob()` 只保留 segment job 入口和 SessionStore 回写协调。
