...
- `playback-timeline/audioSink.ts` 已接管浏览器 HTMLAudioElement 播放实现，并暴露当前 HTMLAudioElement 的 `AudioPlaybackClock`。
- `playback-timeline/audioSink.ts` 已成为真实音频播放适配层；`AdapterAudioRuntime` 不再直接持有 `startAudio / stopAudioRuntime / getAudioPlaybackClock` 散函数，也不再持有 raw HTMLAudioElement 播放实现，而是通过 `PlaybackTimelineAudioSink` 启停音频并读取音频 clock。
...
- `playback-timeline/segmentReleaseSinks.ts` 已接管 queued text/audio segment 的 pending item 取出、turn/message 校验、队列删除和启动副作用触发；pending playback 队列工具也已迁到 `playback-timeline/playbackReleaseQueue.ts`。adapter 侧仍持有 pending 队列和 UI/history/audio playback 副作用函数，但 segment executor 不再直接依赖 adapter facade 内部 release 实现；raw queued-audio consumer 由 `AdapterAudioRuntime.configureSegmentExecution()` 内部注入，app-facing port 只提供段执行配置和 `startSegmentJob()`。
...
- 音频 `duration` / `started` / active-timeline `terminal` 的 SessionStore 写入已移到 `PlaybackTimelineRuntime` 的 audio lifecycle session port；`audioPlaybackStateBridge` 只更新 adapter UI 播放状态，不再直接写这些会话事实。尚未进入 timeline 的 pending audio、缺失 audio_url 等非 timeline 终态仍由 adapter settlement 直接结算。
- motion `started` / active-timeline `terminal` 也已具备 `PlaybackTimelineRuntime` motion lifecycle session port，并且只在 motion sink 仍 open 时写入，避免 late terminal 覆盖已收口的 SessionStore 状态。生产装配已关闭 `PlaybackCompletionCoordinator` 对普通 motion started/terminal 的重复写入；coordinator 不再依赖 motion player 对象或旧 missing-motion timer hook，暂时仍保留 motion playback record、动作段 handoff 和 `playback_finished` flush 协调。
- audio / lip_sync / motion 的 duration / started / terminal 事件不再隐式补注册缺失 sink，也不会写入不匹配的 timeline；如果事件到达时 segment timeline 或对应 sink 缺失，runtime 只记录诊断并丢弃事件，不把错误 wiring 伪装成正常 lifecycle。
...
1. 继续收敛 segment-sink wiring：audio / motion start 已由 `PlaybackTimelineEngine.startSink()` 触发，audio `duration/started/terminal` 和普通 motion `started/terminal` 的 active-timeline SessionStore 写入已进入 `PlaybackTimelineRuntime`；`PlaybackCompletionCoordinator` 已去掉 motion player / legacy timer 依赖，下一步继续降低它的 handoff / record / `playback_finished` 耦合，并让 `PlaybackTimelineRuntime.startSegmentJob()` 继续变薄。
