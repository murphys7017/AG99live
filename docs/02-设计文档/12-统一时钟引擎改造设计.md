...
> 文档状态：演进中。Phase 1 纯逻辑 timeline 已实现；Phase 2 已把真实音频播放接入 timeline；Phase 3 已开始把 motion release context 和 audio-start event 接入 audio timeline；AudioSink / MotionSink / LipSyncSink 适配层已落地，lip sync 已作为 optional sink 进入 PlaybackTimelineEngine 的 terminal 集合；下一步是把剩余 release / event 分发继续收进 timeline。
>
...
- 没有 audio 的 segment 才允许使用显式 synthetic duration；有 audio 但拿不到音频时长/时钟时应暴露诊断，不应伪装成正常同步。
