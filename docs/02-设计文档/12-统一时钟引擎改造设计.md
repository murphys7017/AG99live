...
- `adapter-connection/runtime/audioSettlementController.ts` 已接管 pending audio 查询、缺失音频标 absent、按 turn/all 结算 pending/open audio 的逻辑；`AdapterAudioRuntime` 继续收缩为 queue、timeline、settlement controller 的装配 facade。
