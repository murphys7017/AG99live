...
- `turnPlaybackOrchestratorCore` 已把分散的 text/audio/motion release 回调收敛成带 `text/audio/motion` 槽位的 `releaseSegment(job)`；segment job 执行语义已移动到 `playback-timeline/segmentJobExecutor.ts`，`turn-playback/segmentPlaybackRelease.ts` 只负责把现有 SessionStore / text / audio / motion 端口适配进去。
- `PlaybackTimelineSegmentExecutor` 现在仍调用旧的 text/audio/motion sink 实现，但已经统一决定 motion release context，例如显式 no-audio 时写入 `timelineMode=motion_only`，motion 缺少合法 `receivedAtMs` 时立即失败；下一步可以把它的 audio/text/motion sink 实现替换为 `PlaybackTimelineRuntime` 直接启动 segment job。
