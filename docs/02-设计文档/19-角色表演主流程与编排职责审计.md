...
`hesitation_hold / hesitation_resume`；semantic、gaze、face 与 SpeechPose 只消费这一个窗口，
不新增参数目标、私有时钟、Stage、播放器或 Mixer source。

候选窗口在事件注册前完成合法时间筛选；窗口不足只写入可诊断 decision，不创建随后需要 fallback 的
非法事件。当前内部 trace 为 `performance_schedule_trace.v5`，外部 `engine.motion_intent.v4` 和
...
动态结果直接驱动 `requiredOwnedUntilMs`、sequence staged-part release 和最终 graph duration；单 pose
只使用原有 release 前能够容纳的 residual，不由参数轨道 Stage 重写 plan timing。任何尾部扩展都受
`MAX_MOTION_DURATION_MS - blendOutMs` 限制；semantic transition 本身无法给 blend-out 留出合法预算时
显式编译失败，不截断事件，也不回退到固定值。内部 trace 已升级为
`performance_schedule_trace.v5`，外部 V4/V3 协议、Timeline、ParameterMixer 和 WebSDK 均未改变。
