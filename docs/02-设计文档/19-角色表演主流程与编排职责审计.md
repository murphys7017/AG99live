...
- AG99 Live2D Runtime 的 ActiveParameterMixer 收集原始主动参数贡献，合成最终目标，并作为唯一的
  ParameterPresentation 入口。
...
flowchart TD
    ...
    Q --> R[AG99 Runtime / ActiveParameterMixer]
    N --> S[SpeechSignalRuntime]
    S --> R
    R --> T[AG99 Runtime / ParameterPresentation]
...
-> Drag
-> Breath
-> ActiveParameterMixer 收集 base + raw direct-plan + lip-sync
-> ActiveParameterMixer 合成最终目标并计算 ownership release
...
当前连接级 `TurnCoordinator` 保存最近四个有效动作，并由 Prompt 读取动作形态、主轴方向、`intent_tags`、
