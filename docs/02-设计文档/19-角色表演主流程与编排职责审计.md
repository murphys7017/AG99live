...
1. A1：定义内部 `PerformanceSchedule`，迁移 `SpeechPoseStage` 的 phrase 分析为共享纯函数，并让 SpeechPose 先消费 phrase slots，保持外部行为基本不变。
2. A2：迁移 sequence step 时间分配，生成 estimated alignments，并删除 `parameterTrackGraphCompiler` 对 step start time 的独立所有权。
3. A3：迁移部位 timing policy 和 gesture point 生成，补充 Director/Motion Lab 来源诊断，删除重复时间计算。

A1–A3 完成后再进入阶段 B：gaze、face、hesitation 和动态 residual 策略。Physics setting 级诊断
保持阶段 D，不与阶段 A/B 混合提交。这样每批都保持可审阅和可回退，不会一次性重写整个 ModelEngine。
