...
- [项目结构优化路线图](./项目结构优化路线图.md)
  - 下一阶段结构优化入口。
  - 重点说明为什么先做结构收口，再做动作引擎细节。
- [当前前后端动作链路结构说明](./当前前后端动作链路结构说明.md)
  - 当前前后端文本 / 音频 / 动作链路的结构手册。
  - 当前版本已切到 `TurnPlaybackSession` 为前端播放轮次真源。
- [前端系统模块架构说明](./前端系统模块架构说明.md)
  - 当前前端系统的模块边界、依赖方向和后续收口优先级。
  - 重点说明 Desktop Host、Runtime Bridge、Adapter Protocol、Turn Pipeline、Model Capability、Motion Engine、Avatar Runtime、Authoring Tools 等模块如何协作。
...
- [useAdapterConnection收口实施计划](./archive/useAdapterConnection收口实施计划.md)
- [中间件防御性编程审阅与整改计划](./archive/中间件防御性编程审阅与整改计划.md)
...
- 想了解下一阶段结构优化：读《项目结构优化路线图》。
- 想理解前端总体模块划分：先读《前端系统模块架构说明》。
- 想理解文本 / 音频 / 动作播放链路：先读《当前前后端动作链路结构说明》，再读《文本语音动作同步播放编排设计》。
- 想继续做结构优化：读《项目结构优化路线图》。
- 想理解长期数据归属：读《后端主导数据边界与执行计划》。
