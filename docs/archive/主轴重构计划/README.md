...
2. [02-Profile-Schema与自动扫描](./02-Profile-Schema与自动扫描.md)
3. [03-Prompt语义重写方案](./03-Prompt语义重写方案.md)
...
- `semantic_axis_profile` 的权威来源仍然是后端模型目录
- 前端 `Profile Editor` 只保留未保存草稿
- 已确认有效的动作调参参考样本最终要保存到后端
- few-shot 参考样本池由后端维护并提供给 selector prompt
- `Action Lab` 和 `ModelEngine` 不再长期托管正式事实数据
...
- 第二轮 Profile Editor 主体能力已完成：
  - `Profile Editor` 已从 Action Lab 拆出为独立辅助窗口
  - 可编辑字段：
    - `label`
    - `control_role`
    - `description`
...
- `Action Lab` / `Profile Editor` / runtime snapshot 的工具域与运行时域边界继续收口
- 动作调参参考样本以后端为主的持久化与管理链路
...
在此基础上，下一轮重构不优先改写 `ModelEngine` 执行边界，而是优先继续收口数据归属边界：

```text
Profile canonical source -> 后端
few-shot 正式参考样本 -> 后端
前端 -> 草稿、调试、实时执行
```

补充方案见《[后端主导数据边界与执行计划](../后端主导数据边界与执行计划.md)》。
