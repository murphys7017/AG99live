...
| `turn_id` | 轮次 | 前端创建，后端回传 | 贯穿输入、输出、动作、打断、播放完成、轮次结束 | 所有交互链路消息必须；纯 `system.*` 可为 `null` |
...
`synth_finished` 是 turn 级输出队列关闭信号，不是单条音频生成完成信号。增强版 Core 只可在一条逻辑消息的全部物理发送成功后调用 `complete_visible_message(message_id)`，Adapter 随即声明该 segment；它不等待同一 Turn 的后续 segment。`synth_finished` 到达前，每个已知 segment 必须已经由一个完整 `output.segment` 声明；到达后不接受新段，也不存在晚到 slot 补齐。
...
```json
{
  ...
  "speech": {
    "state": "present",
    "cues": [
      { "kind": "emphasis", "phrase_index": 0, "position": "before" }
    ]
  },
  "images": [],
  "speaker_name": "Alice",
